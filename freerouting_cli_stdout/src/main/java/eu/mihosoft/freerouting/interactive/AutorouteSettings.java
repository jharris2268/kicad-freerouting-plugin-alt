...
public class AutorouteSettings implements java.io.Serializable
{
    ...
    public AutorouteSettings(int p_layer_count)
    {
        ...
        // this can interact with command line args and designforms/specctra/AutoRouteSettings
        max_postroute_passes = 0;
        vias_allowed = true;
        with_fanout = false;
        with_autoroute = true;
        via_costs = 50;
    ...
    public AutorouteSettings(RoutingBoard p_board)
    {
        ...
        set_stop_pass_no(Integer.MAX_VALUE);
	// this can interact with command line args and designforms/specctra/AutoRouteSettings
	// read_scope(Scanner p_scanner, LayerStructure p_layer_structure) which toggles it on/off
        max_postroute_passes = 0;
        vias_allowed = true;
        with_fanout = false;
        with_autoroute = true;
    ...
    {
        // this sets the minimum number of default autoroute passes
    ...
    public void set_max_postroute_passes(int p_value)
    {
        max_postroute_passes = p_value;
    }

    public int get_max_postroute_passes()
    {
        return max_postroute_passes;
    }
    ...
    public void set_dsn_fanout_requested(boolean p_value)
    {
        dsn_fanout_requested = p_value;
    }

    public boolean get_with_fanout()
    {
        if (with_fanout && !dsn_fanout_requested)
        {
	    FRLogger.warn("fanout requested on command line, but dsn did not request fanout");
        }
	else if (!with_fanout && dsn_fanout_requested)
        {
            FRLogger.warn("fanout not requested on command line, but dsn requested fanout");
        }
        return with_fanout;
    }
    ...
    public void set_dsn_autoroute_requested(boolean p_value)
    {
        dsn_autoroute_requested = p_value;
    }

    public boolean get_with_autoroute()
    {
        if ((stop_pass_no > 0) && !dsn_autoroute_requested)
        {
            FRLogger.warn("autoroute on by default, but dsn did not request autoroute");
        }
        return with_autoroute;
    }

    public void set_with_postroute(int p_value)
    {
        max_postroute_passes = p_value;
    }

    public void set_dsn_postroute_requested(boolean p_value)
    {
        dsn_postroute_requested = p_value;
    }

    public boolean get_with_postroute()
    {
        if ((max_postroute_passes > 0) && !dsn_postroute_requested)
        {
            FRLogger.warn("postroute requested on command line, but dsn did not request postroute");
        }
        else if ((max_postroute_passes < 1) && dsn_postroute_requested)
        {
            FRLogger.warn("postroute not requested on command line, but dsn requested postroute");
        }
        return max_postroute_passes > 0;
    }
    ...
    private boolean dsn_fanout_requested;
    private boolean dsn_autoroute_requested;
    private boolean dsn_postroute_requested;
    ...
    private int max_postroute_passes;
