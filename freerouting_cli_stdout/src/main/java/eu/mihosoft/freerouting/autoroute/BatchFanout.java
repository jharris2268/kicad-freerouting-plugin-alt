...
public class BatchFanout
{
    ...
    public static void fanout_board(InteractiveActionThread p_thread)
    {
	    ...
	    FRLogger.progress("fanout_board", i, MAX_PASS_COUNT);
            if (routed_count == 0)
            {
		FRLogger.progress("fanout_board", MAX_PASS_COUNT, MAX_PASS_COUNT);
                break;
            }
        }
    }
...
//            this.thread.hdlg.screen_messages.set_batch_fanout_info(p_pass_no + 1, components_to_go);
        ...
        FRLogger.info("fanout pass: " + (p_pass_no + 1) + ", routed: " + routed_count
