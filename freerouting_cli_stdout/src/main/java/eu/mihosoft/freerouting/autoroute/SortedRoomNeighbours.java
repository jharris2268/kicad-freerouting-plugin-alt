...
public class SortedRoomNeighbours
{
    ...
    public static CompleteExpansionRoom calculate(ExpansionRoom p_room, AutorouteEngine p_autoroute_engine)
    {
        ...
        else
        {
            ...
            {
                FRLogger.debug("AutorouteEngine.calculate_new_incomplete_rooms_with_mmore_than_1_neighbour: unexpected dimension for smoothened_shape");
            }
        }
            ...
            ShapeSearchTree p_autoroute_search_tree, int p_room_id_no, TestLevel p_test_level)
    {
        ...
        for (ShapeTree.TreeEntry curr_entry : overlapping_objects)
        {
            ...
            if (dimension > 1)
            {
                ...
                {
                    FRLogger.debug("SortedRoomNeighbours.calculate: unexpected area overlap of free space expansion room");
                }
                continue;
            }
            if (dimension < 0)
    ...
    private boolean try_remove_edge(int p_net_no, ShapeSearchTree p_autoroute_search_tree, TestLevel p_test_level)
    {
        ...
        if (remove_edge_no >= 0)
        {
            ...
            if (new_rooms.size() != 1)
            {
                FRLogger.debug("AutorouteEngine.calculate_doors: 1 completed shape expected");
                return false;
            }
